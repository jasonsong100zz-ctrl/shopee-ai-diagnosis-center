...
4. 默认选择“离线采集并下载 CSV（推荐）”，无需启动本地桥或连接 Supabase；点击“开始监控”后，结果暂存在 Chrome 本地，任务完成后点击“下载本次结果 CSV”。面板会显示已读取数量、成功/失败数量。遇到 Shopee 验证时人工完成验证，再点“验证后继续”；结束后可点“重试失败”。
5. 如果需要写入云端，切换为“发布到 Supabase”，再点击“检查桥接”，确认显示“本地发布桥正常”，然后开始监控。云端模式才需要启动 `npm run competitor:bridge`；离线模式不会访问 `127.0.0.1:8787`。
5. 任务完成后，点击“下载本次结果 CSV”，Chrome 会让使用者选择保存位置。CSV 同时包含成功采集字段和失败链接，默认文件名为 `FM竞品监控-YYYY-MM-DD.csv`。

CSV 业务结果表按 SKU 一行输出：商品公共字段会在同一商品的 SKU 行中重复，SKU 字段单独保留 `SKU ID`、`SKU名称`、`SKU价格`、`SKU原价`、`SKU库存`、`SKU价格状态` 和 `SKU价格明细`，便于在 Excel 中筛选、排序和比较；`最低SKU价格`、`最高SKU价格`仍是商品级汇总字段。
